...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']) {
        ...
        if ($userType === 'owner' && isset($_POST['rental_id']) && isset($_POST['flat_id'])) {
        ...
        } elseif ($userType === 'owner' && isset($_POST['appointment_id']) && isset($_POST['flat_id'])) {
            $appointment_id = intval($_POST['appointment_id']);
            $flat_id = intval($_POST['flat_id']);
            $action = $_POST['action'];

            // Verify the appointment belongs to the owner's flat
            $check_stmt = $pdo->prepare("
                SELECT a.appointment_id, a.customer_id, f.owner_id, s.appointment_date, s.appointment_time
                FROM appointments a
                JOIN flats f ON a.flat_id = f.flat_id
                JOIN flat_availability_slots s ON a.slot_id = s.slot_id
                WHERE a.appointment_id = :appointment_id AND a.flat_id = :flat_id AND f.owner_id = :owner_id AND a.status = 'pending'
            ");
            $check_stmt->execute([
            ...
            if (!$appointment) {
                $errors[] = "Invalid appointment or not authorized.";
            } else {
                if ($action === 'accept_appointment') {
                    ...
                    $update_slot = $pdo->prepare("
                        ...
                        SET is_booked = TRUE
                        WHERE slot_id = (SELECT slot_id FROM appointments WHERE appointment_id = :appointment_id)
                    ");
                    $update_slot->execute(['appointment_id' => $appointment_id]);

                    // Fetch customer and flat details for the message
                    $details_stmt = $pdo->prepare("
                        SELECT u.name AS customer_name, f.reference_number
                        FROM users u
                        JOIN appointments a ON a.customer_id = u.user_id
                        JOIN flats f ON a.flat_id = f.flat_id
                        WHERE a.appointment_id = :appointment_id
                    ");
                    $details_stmt->execute(['appointment_id' => $appointment_id]);
                    $details = $details_stmt->fetch(PDO::FETCH_ASSOC);
                    ...
                    $message_stmt->execute([
                        ...
                        'message_body' => "Dear {$details['customer_name']}, your appointment request for flat {$details['reference_number']} on {$appointment['appointment_date']} at {$appointment['appointment_time']} has been accepted.",
                        'sender' => "System on behalf of Owner",
                        'flat_id' => $flat_id,
                        'appointment_id' => $appointment_id
                    ]);

                    $success_message = "Appointment request accepted successfully.";
                } elseif ($action === 'reject_appointment') {
                    // Delete associated messages first
                    $delete_messages = $pdo->prepare("DELETE FROM messages WHERE appointment_id = :appointment_id");
                    $delete_messages->execute(['appointment_id' => $appointment_id]);
                    ...
                    $message_stmt = $pdo->prepare("
                        INSERT INTO messages (user_id, title, message_body, sender, sent_date, message_type, flat_id)
                        VALUES (:user_id, :title, :message_body, :sender, NOW(), 'appointment', :flat_id)
                    ");
                    $message_stmt->execute([
                        'user_id' => $appointment['customer_id'],
                        'title' => "Appointment Request Rejected",
                        'message_body' => "Dear {$details['customer_name']}, your appointment request for flat {$details['reference_number']} on {$appointment['appointment_date']} at {$appointment['appointment_time']} has been rejected.",
                        'sender' => "System on behalf of Owner",
                        'flat_id' => $flat_id
                    ]);

                    $success_message = "Appointment request rejected successfully.";
                }
            }
    ...
    $sql = "
        SELECT m.*, f.reference_number, r.status AS rental_status, f.status AS flat_status, a.status AS appointment_status
        FROM messages m
    ...
    if ($userType === 'customer') {
        $sql .= " AND m.message_type IN ('rental', 'appointment')";
    } elseif ($userType === 'owner') {
        $sql .= " AND m.message_type IN ('rental', 'approval', 'appointment')";
                    ...
                    <?php foreach ($messages as $msg): ?>
                        <tr class="<?php echo !$msg['is_read'] ? 'unread' : ''; ?>">
                            ...
                            <td class="action-buttons">
                                ...
                                <?php elseif ($userType === 'owner' && $msg['message_type'] === 'appointment' && isset($msg['appointment_id']) && $msg['appointment_status'] === 'pending'): ?>
                                    <form method="POST" action="">
                                        <input type="hidden" name="appointment_id"
                                               value="<?php echo (int)$msg['appointment_id']; ?>">
                                        <input type="hidden" name="flat_id" value="<?php echo (int)$msg['flat_id']; ?>">
