...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (count($appointment_dates) < 1) {
        ...
        foreach ($appointment_dates as $index => $date) {
            if (!strtotime($date) || !preg_match('/^\d{2}:\d{2}$/', $appointment_times[$index]) || !preg_match('/^\d{10}$/', $telephone_numbers[$index])) {
                ...
                function addSlot() {
                    ...
                    entry.innerHTML = `
                        ...
                        <fieldset class="input-container">
                            ...
                            <input type="text" name="telephone_numbers[]" id="telephone_numbers_${entryCount}"
                                   pattern="\d{10}" maxlength="10" inputmode="numeric"
                                   title="Please enter exactly 10 digits" required>
                        </fieldset>
                        <span class="slot-remove" onclick="removeSlot(this)">Remove</span>
